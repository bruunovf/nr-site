<?php
class saturnthemes_industry_Customizer {
    public static function init() {
        include_once( 'libraries/kirki/kirki.php' );
        add_filter( 'kirki/config', 'saturnthemes_industry_Customizer::config' );
    }
...
if ( ! function_exists( 'saturnthemes_industry_register_legend_control' ) ) {

    add_action( 'customize_register', 'saturnthemes_industry_register_legend_control' );

    function saturnthemes_industry_register_legend_control( $wp_customize ) {
...
add_filter( 'kirki/values/get_value', 'saturnthemes_industry_kirki_db_get_theme_mod_value', 10, 2 );
function saturnthemes_industry_kirki_db_get_theme_mod_value( $value, $setting ) {
    static $settings;
    ...
    if ( is_page() ) {
        ...
        if ( ! empty( $presets ) ) {
            foreach ( $presets as $preset ) {
                $page_preset_value = get_post_meta( get_the_ID(), 'saturnthemes_industry_' . $preset, true );
...
saturnthemes_industry_Customizer::init();
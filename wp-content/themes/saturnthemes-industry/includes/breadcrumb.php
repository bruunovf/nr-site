...
class saturnthemes_bread_crumb {

	public function saturnthemes_industry_bread_crumb( $args = '' ) {

		$default = array(
			'type'             => 'list',
			'home_label'       => __( 'Home', 'saturnthemes-industry' ),
			'search_label'     => __( 'Search Result of "%s"', 'saturnthemes-industry' ),
			'404_label'        => __( '404 Not Found', 'saturnthemes-industry' ),
			'category_label'   => _x( '%s', 'category label', 'saturnthemes-industry' ),
			'tag_label'        => _x( '%s', 'tag label', 'saturnthemes-industry' ),
			'taxonomy_label'   => _x( '%s', 'taxonomy label', 'saturnthemes-industry' ),
			'author_label'     => _x( '%s', 'author label', 'saturnthemes-industry' ),
			'attachment_label' => _x( '%s', 'attachment label', 'saturnthemes-industry' ),
			'year_label'       => _x( '%s', 'year label', 'saturnthemes-industry' ),
			'month_label'      => _x( '%s', 'month label', 'saturnthemes-industry' ),
			'day_label'        => _x( '%s', 'day label', 'saturnthemes-industry' ),
			'post_type_label'  => _x( '%s', 'post type label', 'saturnthemes-industry' ),
			'joint_string'     => __( ' &gt; ', 'saturnthemes-industry' ),
			'navi_element'     => '',
			'elm_class'        => 'saturnthemes_industry_bread_crumb',
			'elm_id'           => '',
			'li_class'         => '',
			'class_prefix'     => '',
		...
		$default = apply_filters( 'saturnthemes_industry_bread_crumb_default', $default );
		...
		$saturnthemes_industry_bread_crumb_arr = $this->get_saturnthemes_industry_bread_crumb_array( $args );
		...
		if ( $args['type'] == 'string' ) {
			$output = array();
			$cnt    = 1;
			foreach ( $saturnthemes_industry_bread_crumb_arr as $ancestor ) {
				if ( $cnt == count( $saturnthemes_industry_bread_crumb_arr ) ) {
			...
			$output = apply_filters( 'saturnthemes_industry_bread_crumb_after', $output, $args );
			...
			$output = apply_filters( 'saturnthemes_industry_bread_crumb_before', $output, $args );
		...
		$output = apply_filters( 'saturnthemes_industry_bread_crumb', $output, $args );
	...
	private function get_saturnthemes_industry_bread_crumb_array( $args ) {
		global $post;

		$saturnthemes_industry_bread_crumb_arr   = array();
		$saturnthemes_industry_bread_crumb_arr[] = array( 'title' => $args['home_label'], 'link' => home_url() . '/' );
		$saturnthemes_industry_bread_crumb_arr   = $this->add_posts_page_array( $saturnthemes_industry_bread_crumb_arr );
		if ( is_404() ) {
			$saturnthemes_industry_bread_crumb_arr[] = array( 'title' => $args['404_label'], 'link' => false );
		} elseif ( is_search() ) {
			$saturnthemes_industry_bread_crumb_arr[] = array(
			...
			if ( $post->post_parent ) {
				if ( $parent_post = get_post( $post->post_parent ) ) {
					$singular_saturnthemes_industry_bread_crumb_arr = $this->get_singular_saturnthemes_industry_bread_crumb_array( $parent_post, $args );
					$saturnthemes_industry_bread_crumb_arr          = array_merge( $saturnthemes_industry_bread_crumb_arr, $singular_saturnthemes_industry_bread_crumb_arr );
				}
			}
			$saturnthemes_industry_bread_crumb_arr[] = array(
			...
			$singular_saturnthemes_industry_bread_crumb_arr = $this->get_singular_saturnthemes_industry_bread_crumb_array( $post, $args );
			$saturnthemes_industry_bread_crumb_arr          = array_merge( $saturnthemes_industry_bread_crumb_arr, $singular_saturnthemes_industry_bread_crumb_arr );
			$saturnthemes_industry_bread_crumb_arr[]        = array( 'title' => $post->post_title, 'link' => get_permalink( $post->ID ) );
		} elseif ( is_category() ) {
			global $cat;
		...
		return apply_filters( 'saturnthemes_industry_bread_crumb_arr', $saturnthemes_industry_bread_crumb_arr, $args );
	}


	private function get_singular_saturnthemes_industry_bread_crumb_array( $post, $args ) {
		$saturnthemes_industry_bread_crumb_arr = array();
		...
		return $saturnthemes_industry_bread_crumb_arr;
	}


	private function add_posts_page_array( $saturnthemes_industry_bread_crumb_arr ) {
...
$saturnthemes_bread_crumb = new saturnthemes_bread_crumb();

if ( ! function_exists( 'saturnthemes_industry_bread_crumb' ) ) {
	function saturnthemes_industry_bread_crumb( $args = '' ) {
		global $saturnthemes_bread_crumb;

		return $saturnthemes_bread_crumb->saturnthemes_industry_bread_crumb( $args );
	}
}
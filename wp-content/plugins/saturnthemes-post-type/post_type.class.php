...
add_action( 'init', array( 'SaturnThemes_PostType', 'init' ) );

class SaturnThemes_PostType {
	...
	public static function registerPostType( $postType, $title, $args ) {
		...
		$labels      = array(
			'name'               => __( $pluralTitle, SATURNTHEMES__POST_TYPE ),
			'singular_name'      => __( $title, SATURNTHEMES_POST_TYPE ),
			'add_new'            => __( 'Add New', SATURNTHEMES_POST_TYPE ),
			'add_new_item'       => __( 'Add New ' . $title, SATURNTHEMES_POST_TYPE ),
			'edit_item'          => __( 'Edit ' . $title, SATURNTHEMES_POST_TYPE ),
			'new_item'           => __( 'New ' . $title, SATURNTHEMES_POST_TYPE ),
			'all_items'          => __( 'All ' . $pluralTitle, SATURNTHEMES_POST_TYPE ),
			'view_item'          => __( 'View ' . $title, SATURNTHEMES_POST_TYPE ),
			'search_items'       => __( 'Search ' . $pluralTitle, SATURNTHEMES_POST_TYPE ),
			'not_found'          => __( 'No ' . $pluralTitle . ' found', SATURNTHEMES_POST_TYPE ),
			'not_found_in_trash' => __( 'No ' . $pluralTitle . '  found in Trash', SATURNTHEMES_POST_TYPE ),
			'parent_item_colon'  => '',
			'menu_name'          => __( $pluralTitle, SATURNTHEMES_POST_TYPE )
		);
	...
	public static function addMetaBox( $id, $title, $post_types, $callback = '', $context = '', $priority = '', $callback_args = '' ) {
		...
		foreach ( $post_types as $post_type ) {
			$title                                     = empty( $title ) ? __( 'Options', SATURNTHEMES_POST_TYPE ) : $title;
	...
	public static function display_metaboxes( $post, $metabox ) {
		...
		echo '<input type="hidden" name="saturnthemes_custom_nonce" value="' . wp_create_nonce( basename( __FILE__ ) ) . '" />';
		echo '<table class="form-table saturnthemes">';
		foreach ( $fields as $key => $field ) {
			...
			switch ( $field['type'] ) {
				...
				case 'table_two_column':
					...
					echo '<div class="saturnthemes_table-two-columns">';

						if($meta) {
							foreach($meta as $t_key => $t_value) {
								echo '<div class="saturnthemes-table_row" row-id="'.$t_key.'">';
					...
					echo '<script>
						    jQuery(function($) {
								...
								$(".add-row").on("click", function() {
									if($(".saturnthemes-table_row").length) {
										i = parseInt($(".saturnthemes-table_row").last().attr("row-id")) + 1;
									}

									$(".saturnthemes_table-two-columns").append(
										"<div class=\"saturnthemes-table_row\" row-id="+i+">" +
				...
				case 'checkbox':
					...
					break;
				case 'images':
					echo '<div class="saturnthemes-metabox-media">';
					     if($meta) {
						     foreach($meta as $array_key => $val) {
							     $image = wp_get_attachment_image_src($val, 'medium');
							     $image = $image[0];
							     echo '<div class="saturnthemes-uploaded-file">'.
							          '<img src="'.$image.'"/>'.
							          '<input type="hidden" class="saturnthemes-upload-field" value="'.$val.'" name="'.$key.'['.$array_key.']" />'.
							          '</div>';
						     }
					     }
					echo '</div>'.
						 '<div class="saturnthemes-add-media button button-primary">'.__( "Add", SATURNTHEMES_POST_TYPE ).'</div>'.
						 '<script>
						    jQuery(function($) {
						       var insertImage = wp.media.controller.Library.extend({
								...
								frame.on( "select",function() {
								    ...
								    $(".saturnthemes-metabox-media").html("");
								    ...
								    selection.each(function(attachment) {
								        ...
								        var attachmentHtml = attachment.attributes["html"],
								            ...
								            $(".saturnthemes-metabox-media").append("<div class=\"saturnthemes-uploaded-file\"><img src="+attachment.attributes["attachment"]["url"]+" /> <input type=\"hidden\" value="+attachmentID+" name=\"'.$key.'["+i+"] \" /></div>");
								            i++;
								...
								frame.on("open",function() {
								    ...
								    $(".saturnthemes-uploaded-file").find("input[type=\"hidden\"]").each(function(){
								         var input_id = $(this);
								        if( input_id.val() ){
								...
								$(".saturnthemes-add-media").on("click", function() {
									frame.open();
								});
				...
				case 'date_picker':
					...
					echo '<input class="form-control" id="saturnthemes-timedatetimepicker-'.$key.'" name="' . $key . '"  value="' . $meta . '" />
					     <script type="text/javascript">
						     jQuery(document).ready(function($){
								$("#saturnthemes-timedatetimepicker-'.$key.'").saturnthemes_datetimepicker({
				...
				case 'image':
					...
					echo '
						<div class="saturnthemes_metabox_image">
							<input name="'. $key .'" type="hidden" class="custom_upload_image" value="'. $meta .'" />
							<img src="'. $image .'" class="custom_preview_image" alt="" />
							<input class="saturnthemes_upload_image upload_button_'. $key .' button-primary" type="button" value="' . __( 'Choose Image', SATURNTHEMES_POST_TYPE ). '" />
							<a href="#" class="saturnthemes_remove_image button">' . __( 'Remove Image', SATURNTHEMES_POST_TYPE ). '</a>
						</div>
						<script type="text/javascript">
							jQuery(function($) {
								$(".upload_button_'. $key .'").click(function(){
									...
									var custom_uploader = wp.media({
										title   : "' . __( 'Select image', SATURNTHEMES_POST_TYPE ) . '",
										button  : {
											text: "' . __( 'Attach', SATURNTHEMES_POST_TYPE ) . '"
										},
										multiple: true
									}).on("select", function () {
										var attachment = custom_uploader.state().get("selection").first().toJSON();
										btnClicked.closest(".saturnthemes_metabox_image").find(".custom_upload_image").val(attachment.id);
										btnClicked.closest(".saturnthemes_metabox_image").find(".custom_preview_image").attr("src", attachment.url);

									}).open();
								});
								$(".saturnthemes_remove_image").click(function(){
									$(this).closest(".saturnthemes_metabox_image").find(".custom_upload_image").val("");
									$(this).closest(".saturnthemes_metabox_image").find(".custom_preview_image").attr("src", "' . $default_image . '");
									return false;
								});
							});
						</script>
	...
	public static function save_metaboxes( $post_id ) {
		...
		if ( ! isset( $_POST['saturnthemes_custom_nonce'] ) ) {
			return $post_id;
		}
		if ( ! wp_verify_nonce( $_POST['saturnthemes_custom_nonce'], basename( __FILE__ ) ) ) {
			return $post_id;
		}
	...
	public static function addTaxonomy( $slug, $taxonomyName, $post_type, $args = '' ) {
		...
		$labels     = array(
			'name'              => _x( $taxonomyName, 'taxonomy general name', 'saturnthemes_theme' ),
			'singular_name'     => _x( $taxonomyName, 'taxonomy singular name', 'saturnthemes_theme' ),
			'search_items'      => __( 'Search ' . $pluralName, 'saturnthemes_theme' ),
			'all_items'         => __( 'All ' . $pluralName, 'saturnthemes_theme' ),
			'parent_item'       => __( 'Parent ' . $taxonomyName, 'saturnthemes_theme' ),
			'parent_item_colon' => __( 'Parent ' . $taxonomyName . ':', 'saturnthemes_theme' ),
			'edit_item'         => __( 'Edit ' . $taxonomyName, 'saturnthemes_theme' ),
			'update_item'       => __( 'Update ' . $taxonomyName, 'saturnthemes_theme' ),
			'add_new_item'      => __( 'Add New ' . $taxonomyName, 'saturnthemes_theme' ),
			'new_item_name'     => __( 'New ' . $taxonomyName . 'Name', 'saturnthemes_theme' ),
			'menu_name'         => __( $taxonomyName, 'saturnthemes_theme' )
		);
